...
def add():
    if request.method == 'POST':
        ...
        return_date_raw = request.form.get('returnDate')
        returnDate = datetime.strptime(return_date_raw, '%Y-%m-%d') if return_date_raw else datetime.now() + timedelta(days=14)
...
def return_book(id):
    ...
    if loan.returnDate is not None and datetime.now() > loan.returnDate:
